...
export function setupDevtoolsPlugin<
  ...
  TSettings = ExtractSettingsTypes<
    TDescriptor extends { settings: infer S }
...
>(pluginDescriptor: Narrow<TDescriptor>, setupFn: SetupFunction<TSettings>) {
  ...
  if (
    hook &&
    (target.__DOCUE_DEVTOOLS_PLUGIN_API_AVAILABLE__ || !enableProxy)
  ) {
    hook.emit(HOOK_SETUP, pluginDescriptor, setupFn)
  } else {
    const proxy = enableProxy ? new ApiProxy(descriptor, hook) : null

    const list = (target.__DOCUE_DEVTOOLS_PLUGINS__ =
      target.__DOCUE_DEVTOOLS_PLUGINS__ || [])
    list.push({
